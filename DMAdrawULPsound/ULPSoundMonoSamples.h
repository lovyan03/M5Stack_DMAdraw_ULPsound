...
long pos = 0;
volatile unsigned char nextSample()
{
  //if (M5.BtnC.isReleased())    return 128; 

  if(pos >= sampleCount)
    pos = 0;

  return (unsigned char)((int)samples[pos++] * volume / 255 + 128);
}

int lastFilledWord = 0;

volatile void fillSamples()
{
  ...
  while(lastFilledWord != currentWord)
  {
    ...
    lastFilledWord = ++lastFilledWord == totalSampleWords ? 0 : lastFilledWord;
  }
}
public:
  void setup() 
