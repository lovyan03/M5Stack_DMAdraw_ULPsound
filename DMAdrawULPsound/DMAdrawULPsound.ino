...
void lcd_init(spi_device_handle_t spi)
{
    ...
    //detect LCD type
    uint32_t lcd_id = 0; // lcd_get_id(spi);
...
static void display_pretty_colors(spi_device_handle_t spi)
{
    ...
    for (int y=0; y<240; y+=PARALLEL_LINES) {
        //Calculate a line.
        for ( int p = 0; p < 320 * PARALLEL_LINES; ++p) {
          lines[calc_line][p] = frame * (PARALLEL_LINES + 1) + (p >> 2) + (y >> 2);
//        lines[calc_line][p] = (frame & 1) ? 0xffff : 0;
        }
...
void loop(void)
{
    ...
    uint8_t volume { ulp.getVolume() } ;
    if (digitalRead(39) == 0 && volume > 0) {
      ulp.setVolume(--volume);
    } 
    if (digitalRead(38) == 0 && volume < 255) {
      ulp.setVolume(++volume);
    }
